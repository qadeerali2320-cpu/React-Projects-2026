...
export default function Textform(props) {
  ...
  const HandleSpeak = () => {
    if (text.trim() == "") {
      ...
      props.showAlert("No text to listen", "warning");
      return;

    }
    if (listen == false) {
      setText("Speech Is stopped ");
      props.showAlert("Speech is stopped", "success");
      return;
    }
  ...
  return (
    <>
      ...
      >
        ...
        <button className="btn btn-success  my-2 mx-1" onClick={HandleClearText}>Clear Text</button>
        <button className="btn btn-success my-2 mx-1" onClick={HandleEmailText}> Email In text</button>
        <button className="btn btn-success  my-2 mx-1" onClick={HandleSpeak}>Listen to Text</button>
        <button className="btn btn-success my-2 mx-1" onClick={HandleStopListening}>Stop Listening to Text</button>
        <button className="btn btn-success  my-2 mx-1" onClick={HandleLowClick}>Convert To Lower Case</button>
      ...
      <div className="container2 my-3" style={{ color: props.mode === 'dark' ? 'white' : 'black' }}>
        ...
        <p> Average time to read the text is {0.008 * text.split(" ").filter((element)=>{return element.length!==0}).length}</p>
